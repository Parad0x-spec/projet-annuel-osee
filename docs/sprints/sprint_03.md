...
La neuvième tâche est l'implémentation côté tablette de la lecture du champ `niveau_demande` reçu dans le payload `creation_patient` et de son application stricte au démarrage de la session de jeu. Cela couvre la validation que la valeur est bien un entier compris entre 1 et 5 conformément à la spec QR, le mapping de ce niveau vers les paramètres concrets du jeu des émotions tels que définis dans `docs/specs/jeu_emotions.md`, et la propagation de la configuration jusqu'au lancement de la planche. La tablette n'effectue aucun calcul d'adaptation et ne consulte aucun historique, elle applique tel quel le niveau décidé par le praticien. Tests unitaires obligatoires sur le mapping niveau vers paramètres et sur le rejet d'un niveau hors plage.
